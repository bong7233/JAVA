...
public class FileInputStreamTest1 {
	...
	public static void main(String[] args) {
		...
		try {
			fis = new FileInputStream("input.txt"); // "ABC"담긴 txt
...
public class FileInputStreamTest2 {
	...
	public static void main(String[] args) {
		...
		try(FileInputStream fis = new FileInputStream("input.txt")){ 
			int i;
			while ( (i = fis.read()) != -1){ // 파일의끝이 -1을 나타내므로
				System.out.println((char)i); // ABC
			}
			System.out.println("end");
...
public class FileInputStreamTest3 {
	...
	public static void main(String[] args) {
		...
		try (FileInputStream fis = new FileInputStream("input2.txt")){ // "A-Z" txt 파일
			
			byte[] bs = new byte[10]; // byte array 10짜리 생성
			int i;
			while ( (i = fis.read(bs)) != -1){ // 파일 끝까지 read
				for(int k= 0; k<i; k++){
...
- 많은 추상 메서드가 선언되어 있고 이를 하위 스트림이 상속받아 구현함
- 주요 하위 클래스
- InputStream은 파일이 없을때 오류가 생기지만 OutputStream은 없을때 생성함
  - 하지만 두번 호출시에 덮어쓰기되므로 두번쨰 매개변수로 True를 넣어줘야함
  - new FileOutputStream("파일이름", True) 형태
...
public class FileOutputStreamTest2 {
	...
	public static void main(String[] args) throws IOException {
        ...
        try(fos){ //java 9 부터 제공되는 기능
			...
			fos.write(bs);  // ABC...YZ -> 배열 한꺼번에 출력하기
		}catch(IOException e) {
			e.printStackTrace();
		}
		System.out.println("출력이 완료되었습니다.");
...
public class FileOutputStreamTest3 {
	...
	public static void main(String[] args) {
		...
		try(FileOutputStream fos = new FileOutputStream("output3.txt"))
		{
			...
			fos.write(bs, 2, 10);   // CDEFGHIJKL -> 배열의 2 번째 위치부터 10 개 바이트 출력하기 
		}catch(IOException e) {
			e.printStackTrace();
		}
		System.out.println("출력이 완료되었습니다.");
